...
class BicepCurl(BaseExercise):
    """
    Rep-counting and form-checking logic for the bicep curl exercise.
    """
    def __init__(self):
        super().__init__()
        self.feedback = "" 
    ...
    def check(self, landmarks):
        ...
        and returns the current elbow angle and rep counter.
        """
        if self.target_goal == 0:
            self.feedback = ""
            return 0, self.counter
            
        shoulder_lm = landmarks[11] # working shoulder
        other_shoulder_lm = landmarks[12] # other shoulder, used to check body alignment
        elbow_lm = landmarks[13]
        wrist_lm = landmarks[15]
        
        # 1. Basic, lenient visibility check
        ...
        # Current elbow angle (shoulder -> elbow -> wrist), used for rep counting
        ...
        # Shoulder abduction angle (other shoulder -> working shoulder -> elbow)
        ...
        # 2. Guard A: block vertical arm swinging (elbow height rule on the Y axis)
        is_elbow_too_high = elbow_lm.y < (shoulder_lm.y + 0.03)

        # 3. Guard B (strict): block cheating by flaring the elbow out to the side
        ...
        # 4. Strict, controlled rep-counting logic, plus the quiet bonus-mode counter
        if not self.has_welcomed:
            ...
            if angle > 130: 
                ...
                if self.target_goal > 0 and not self.goal_finished:
                    ...
                    # Quiet update: shows the current bonus count without re-announcing it out loud
                ...
                if self.target_goal > 0 and not self.goal_finished:
                    ...
                    if current_left == 0:
                        ...
                        # Fires only once, when switching into bonus mode
                        self.feedback = "Bonus mode active!"
                        print("\n[LOG] Target goal reached! Switching to Bonus Mode.\n")
                    else:
                        self.feedback = "Good rep!"
                else:
                    # Subsequent bonus reps: short feedback with just the count, e.g. "Plus 2", "Plus 3"
